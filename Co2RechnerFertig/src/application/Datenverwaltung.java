...
public class Datenverwaltung {
	...
	// Methode um die Nutzereingaben in der Datenbank zu speichern
		...
		public void updateEntry(Connection connection, String tableName, String sqlQuery, Object... parameters) {
		    try (PreparedStatement preparedStatement = connection.prepareStatement(sqlQuery)) {
		        for (int i = 0; i < parameters.length; i++) {
		            if (parameters[i] instanceof Integer) {
		                preparedStatement.setInt(i + 1, (Integer) parameters[i]);
		        ...
		        // Führe den SQL-Befehl aus, um die Daten in der Datenbank zu aktualisieren
		        preparedStatement.executeUpdate();
		    } catch (SQLException e) {
		        e.printStackTrace();
		        // Hier können Sie eine geeignete Fehlerbehandlung implementieren
	...
	// Methode um die Daten aus der Datenbank abzurufen
		public static List<Object[]> fetchAllDataFromTable(Connection connection, String tableName, String[] columns) throws SQLException {
	...
	//Methode um die eingegebene Anzahl an Personen abrufen zu können.
	public int getLastNumberOfPersons() {
	    String selectSql = "SELECT personen FROM housing ORDER BY ID DESC LIMIT 1"; 
	    
	    try (PreparedStatement preparedStatement = connection.prepareStatement(selectSql);
	        ResultSet resultSet = preparedStatement.executeQuery()) {
	        
	        if (resultSet.next()) {
	            return resultSet.getInt("personen");
	        }
	    } catch (SQLException e) {
	        e.printStackTrace();
	    }
